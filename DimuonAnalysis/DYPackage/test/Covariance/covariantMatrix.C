...
void writeEffCorrToRootFile(std::string resultType  = "1D", std::string channel = "mu"){
  ...
  if("2D"!=resultType){
    if("mu"==channel){
      file_name =  "effCorr_12.09.13.root";
    }
    else{
      file_name = "dyee_effCorrelations_1D-20121122.root";
    }
  }
  ...
  if("mu"!=channel){
    effHisto_name = "effCorrelation_bins_NORM";
  }
  ...
  if("mu"!=channel){
    effHisto_name = "effCorrelation_bins";
  }
...
//
void writeShapeToRootFile(std::string resultType  = "1D", std::string channel = "mu", 
  ...
  std::string inputFileName = "r2D_preFSR_inAcc_mu_26.03.13.dat";
  if("1D"==resultType){
    if("mu"==channel){
      if("fullAcc" == acc){
	if("preFSR" == fsr){
	  inputFileName = "r1D_preFSR_fullPhS_mu_12.09.13.dat";
	}
	else{
...
void writeTheoryToRootFile(std::string resultType  = "2D", std::string channel = "mu", 
...
void writeSystToRootFile(std::string resultType  = "1D", std::string channel = "mu",
			 std::string acc = "fullAcc", std::string fsr = "preFSR"){
  ...
  std::string inputFileName = "r2D_syst_preFSR_inAcc_mu_26.03.13.dat";
  directoryName = "./";
  inputFileName = "r2D_syst_preFSR_inAcc_mu_26.03.13.dat";
  if("1D"==resultType){
    if("mu"==channel){
      if("fullAcc" == acc){
	if("preFSR" == fsr){
	  inputFileName = "r1D_syst_preFSR_fullPhS_mu_12.09.13.dat";
	  directoryName = "./";
	}
	else{
...
//
void estimateCovMatrix(std::string resultType  = "1D", std::string channel = "mu",
		       std::string acc = "fullAcc", std::string fsr = "preFSR", std::string resultR = "normalized"){// "2D", "1D"
  ...
  TFile *f1=
    (TFile*)gROOT->GetListOfFiles()->FindObject(file_name.c_str());
  TFile *f1 = new TFile(file_name.c_str());
  file_name = "resMatrixProd_2Dunfold_corrRun2011AB.root";
  if("1D"==resultType){
    if("mu"==channel){
      file_name = "massResolUnfold.root";
    }
    else{
  ...
  if("1D"==resultType){
    if("mu"==channel){
      file_name = "FSRresMatrixProd_tmp.root";
    }
    else{
      //file_name = "dyee_fsrResponse_1D-20121113.root";
      file_name = "dyee_fsrResponse_1D-20121122.root";
    }
  }
  ...
  //TH2D * h2_responceMatrix = (TH2D*)(*(DataFiles[1])).Get(matrixName.c_str());
  //TH2F* hmig = (TH2F*)(*(DataFiles[0])).Get("hden");
  //TH1F* _inTrue = (TH1F*)(*(DataFiles[0])).Get("htrue");

  TH2F* hmig = (TH2F*)(*(DataFiles[1])).Get("hden");
  TH1F* _inTrue = (TH1F*)(*(DataFiles[1])).Get("hpreFSR");

  TH2F* h2_responceMatrix  = getMigMatrix(_inTrue, hmig);
  if("el"==channel){
    matrixName = "detResp";
  }
  ...
  */
  ...
  matrixName = "migMatrix";
  TH2D * h2_responceMatrix_FSR = (TH2D*)(*(DataFiles[2])).Get(matrixName.c_str());
  TMatrixT <double>  responseMatrix_FSR(nBins,nBins);
  //std::cout<<" h2_d1->GetNbinsX() = "<<h2_d1->GetNbinsX()<<std::endl;
  ...
  */
  ...
  for(int i =0;i<nBins;++i){
    for(int j =0;j<nBins;++j){
      ...
      //if(i<2 && j<2){
      //std::cout<< i+1 <<" "<< j+1<<" "<< effCov_TM[i][j]<<std::endl;
      //}
    }
  }
  //effErr_TV.Draw();
...
void estimateChi2(int binsRegion_min, int binsRegion_max, std::string resultType  = "2D", std::string channel = "mu",
		       ...
		       std::string resultR = "normalized", bool useTheoryErrors =true){// "2D", "1D"
